...
const { requiredRule, positiveAmountRule } = useValidationRules()
...
<template>
  <div class="mb-8">
    <v-btn class="text-none mb-4 px-0" color="primary" data-testid="transfer-back-link" prepend-icon="mdi-arrow-left"
      to="/transactions/" variant="text">
  ...
  <v-card border class="pa-6 pa-sm-8" flat rounded="lg">
    <v-row justify="center">
      <v-col cols="12" lg="5" md="8">
        <v-alert v-if="apiError" class="mb-4" color="error" data-testid="transfer-api-error" density="compact"
          type="error" variant="tonal">
          {{ apiError }}
        </v-alert>

        <!-- ── Step 1: Form ── -->
        <v-form v-show="step === 'form'" ref="formRef" @submit.prevent="reviewTransfer">
          <!-- Transfer Type -->
          <div class="text-overline font-weight-bold text-grey-darken-1 mb-2">
            {{ $t('transfers.transferType') }}
          </div>
          <v-btn-toggle v-model="transferType" class="mb-4 w-100 d-flex" color="primary"
            data-testid="transfer-type-toggle" density="comfortable" mandatory rounded="lg" variant="outlined">
          ...
          <v-select v-model="form.sender_wallet_id" data-testid="transfer-sender-wallet" density="comfortable"
            :disabled="walletsLoading" :error-messages="errors.sender_wallet_id" hide-details="auto" item-value="id"
            :items="senderWalletItems" :loading="walletsLoading" :rules="[requiredRule]" variant="outlined">
                ...
                <template #title>
                  <div class="d-flex align-center justify-space-between w-100">
                    <span :class="{ 'text-grey': item.raw.disabled }">
                      {{ item.raw.name }}
                      <v-chip v-if="item.raw.disabled" class="ml-2" color="error" density="compact" size="x-small">🔒
                        Frozen</v-chip>
                    </span>
          ...
          <template v-if="transferType === 'external'">
            <div class="d-flex align-center justify-space-between mb-2 mt-4">
              ...
              <a class="text-caption text-primary text-decoration-none cursor-pointer"
                data-testid="transfer-address-book-link" @click="addressBookDialog = true">
                {{ $t('addressBook.title') }}
              </a>
            </div>
            <v-text-field v-model="form.external_name" class="mb-1" data-testid="transfer-external-name"
              density="comfortable" :error-messages="errors.external_name" hide-details="auto"
              :label="$t('transfers.recipientName')" :placeholder="$t('transfers.recipientNamePlaceholder')"
              :rules="[requiredRule]" variant="outlined" />
            <v-text-field v-model="form.external_address" class="mt-2" data-testid="transfer-external-address"
              density="comfortable" :error-messages="errors.external_address" hide-details="auto"
              :label="$t('transfers.externalAddress')" :placeholder="$t('transfers.externalAddressPlaceholder')"
              :rules="[requiredRule]" variant="outlined" />
          </template>
          ...
          <template v-if="transferType === 'internal'">
            ...
            <v-select v-model="form.receiver_wallet_id" data-testid="transfer-receiver-wallet" density="comfortable"
              :error-messages="errors.receiver_wallet_id" hide-details="auto" item-value="id"
              :items="receiverWalletOptions" :loading="walletsLoading" :rules="[requiredRule]" variant="outlined">
              <template #item="{ item, props: itemProps }">
                <v-list-item v-bind="itemProps" :disabled="item.raw.disabled">
                  <template #title>
                    <div class="d-flex align-center justify-space-between w-100">
          ...
          <v-text-field v-model.number="form.amount" data-testid="transfer-amount" density="comfortable"
            :error-messages="errors.amount" hide-details="auto" min="0.01" :prefix="currencySymbol" :rules="amountRules"
            step="0.01" type="number" variant="outlined" />

          <v-alert v-if="exceedsThreshold" class="mt-3" :color="bypassesThreshold ? 'info' : 'warning'"
            data-testid="transfer-threshold-warning" density="compact" :type="bypassesThreshold ? 'info' : 'warning'"
            variant="tonal">
            {{ bypassesThreshold
              ? $t('transfers.thresholdAutoApproved', {
                amount: formatCurrency(currentThreshold ?? 0,
                  selectedWallet?.currency ??
                  'USD')
              })
              : $t('transfers.thresholdWarning', {
                amount: formatCurrency(currentThreshold ?? 0, selectedWallet?.currency
                  ?? 'USD')
              })
            }}
          </v-alert>
          ...
          <v-text-field v-model="form.reference" data-testid="transfer-reference" density="comfortable"
            :error-messages="errors.reference" hide-details="auto" :placeholder="$t('transfers.referencePlaceholder')"
            :rules="referenceRules" variant="outlined" />
          ...
          <template v-if="exceedsThreshold">
            ...
            <v-textarea v-model="form.notes" data-testid="transfer-notes" density="comfortable"
              :error-messages="errors.notes" hide-details="auto" :placeholder="$t('transfers.notesPlaceholder')"
              rows="2" variant="outlined" />
          </template>

          <!-- Form Actions -->
          <div class="d-flex ga-4 mt-6">
            <v-btn class="text-none font-weight-bold px-8" color="primary" data-testid="transfer-submit-btn" height="48"
              rounded="lg" @click="reviewTransfer">
              {{ $t('transfers.submit') }}
            </v-btn>
            <v-btn class="text-none font-weight-bold px-8" color="grey-darken-1" data-testid="transfer-cancel-btn"
              height="48" rounded="lg" to="/transactions/" variant="outlined">
              {{ $t('common.cancel') }}
            </v-btn>
          </div>
        </v-form>
        ...
        <div v-if="step === 'recap'" data-testid="transfer-recap">
          <v-list class="pa-0" lines="two">
            ...
            <v-list-item class="px-0">
              ...
              <div v-if="transferType === 'external' && form.external_address"
                class="text-caption text-grey-darken-2 mt-1" style="font-family: monospace">
          ...
          <v-alert v-if="exceedsThreshold" class="mt-4" :color="bypassesThreshold ? 'info' : 'warning'"
            density="compact" :type="bypassesThreshold ? 'info' : 'warning'" variant="tonal">
            {{ bypassesThreshold
              ? $t('transfers.thresholdAutoApproved', {
                amount: formatCurrency(currentThreshold ?? 0,
                  selectedWallet?.currency ??
          ...
          <!-- Recap Actions -->
          <div class="d-flex ga-4 mt-6">
            <v-btn class="text-none font-weight-bold px-8" color="primary" data-testid="transfer-confirm-btn"
              height="48" :loading="processing" rounded="lg" @click="confirmTransfer">
              {{ $t('transfers.initiateTransfer') }}
            </v-btn>
            <v-btn class="text-none font-weight-bold px-8" color="grey-darken-1" data-testid="transfer-back-btn"
              :disabled="processing" height="48" rounded="lg" variant="outlined" @click="goBack">
              {{ $t('common.goBack') }}
            </v-btn>
          </div>
        </div>
  ...
  <!-- Address Book Dialog -->
  <AddressBookDialog v-model="addressBookDialog" @select="onAddressBookSelect" />
  ...
  <IdentityConfirmDialog v-model="identity.showDialog.value" v-model:credential="identity.credential.value"
    ...
    :is-submitting="identity.isSubmitting.value" @cancel="identity.cancel()" @confirm="identity.confirm()" />
</template>
