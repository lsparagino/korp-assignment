...
const fetchDashboardData = async () => {
    try {
        ...
        totalBalances.value = response.data.balances.map((b: any) => ({
            amount: new Intl.NumberFormat('en-US', { style: 'currency', currency: b.currency }).format(b.amount),
            currency: b.currency
        }));
...
<template>
    ...
    <!-- Total Balance Card -->
    <v-card flat border rounded="lg" class="mb-6 pa-6" :loading="processing">
    ...
    <v-row class="mb-6">
        <v-col v-for="wallet in wallets" :key="wallet.name" cols="12" md="4">
            <v-card flat border rounded="lg" class="pa-4">
                ...
                <div>
                    <span class="text-h5 font-weight-black mr-2">{{ wallet.balanceFormatted }}</span>
                ...
                <tr v-for="item in transactions" :key="item.date + item.to">
                    ...
                    <td>
                        <div class="d-flex align-center">
                            <span class="text-caption text-grey-darken-2 font-weight-medium">{{ item.to }}</span>
                        </div>
                    </td>
                    <td class="text-grey-darken-2">{{ item.type }}</td>
                    <td :class="`text-${item.amountColor} font-weight-bold`">{{ item.amountFormatted }}</td>
                </tr>
            </tbody>
        </v-table>
