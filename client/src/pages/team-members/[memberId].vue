...
<template>
  <div class="mb-6">
    <v-btn class="text-none mb-4 px-0" color="primary" prepend-icon="mdi-arrow-left" to="/team-members/" variant="text">
    ...
    <div class="d-flex align-center ga-3">
      ...
      <v-chip v-if="member" class="text-uppercase font-weight-bold" :color="getRoleColors(member.role).bg" size="small"
        variant="flat">
    ...
    <v-row>
      ...
      <v-col cols="12" lg="7">
        <v-card border class="pa-6" flat rounded="lg">
          <!-- Admin: editable form -->
          <v-form v-if="authStore.isAdmin" ref="formRef" v-model="formValid" @submit.prevent="save">
            <div class="d-flex flex-column ga-4">
              <v-text-field v-model="form.name" data-testid="member-name-input" :error-messages="errors.name"
                :label="$t('common.fullName')" :placeholder="$t('teamMembers.fullNamePlaceholder')"
              ...
              <v-text-field v-model="form.email" data-testid="member-email-input" :error-messages="errors.email"
                :label="$t('common.emailAddress')" :placeholder="$t('teamMembers.emailPlaceholder')"
                :rules="[requiredRule]" type="email" variant="outlined" />
              ...
              <div class="d-flex ga-4 mt-2">
                <v-btn class="text-none font-weight-bold px-8" color="primary" data-testid="member-save-btn"
                  :disabled="!formValid" height="48" :loading="processing" rounded="lg" type="submit">
                  {{ $t('teamMembers.updateMember') }}
                </v-btn>
                <v-btn class="text-none font-weight-bold px-8" color="grey-darken-1" :disabled="processing" height="48"
                  rounded="lg" to="/team-members/" variant="outlined">
                  {{ $t('common.cancel') }}
                </v-btn>
              </div>
            </div>
          ...
          <div v-else class="d-flex flex-column ga-4">
            <div>
              <div class="text-caption text-grey-darken-1">{{ $t('common.fullName') }}</div>
              <div class="text-body-1 font-weight-medium">{{ member.name }}</div>
            </div>
            <div>
              <div class="text-caption text-grey-darken-1">{{ $t('common.emailAddress') }}</div>
              <div class="text-body-1 font-weight-medium">{{ member.email }}</div>
            </div>
            <div>
              <div class="text-caption text-grey-darken-1">{{ $t('teamMembers.walletAccess') }}</div>
              <div class="d-flex flex-wrap ga-2 mt-1">
                <v-chip v-for="w in wallets.filter(w => member?.assigned_wallets?.includes(w.id))" :key="w.id"
                  size="small" variant="tonal">
                  {{ w.name }} ({{ w.currency }})
                </v-chip>
                <span v-if="!member.assigned_wallets?.length" class="text-body-2 text-grey-darken-1">—</span>
              </div>
            </div>
          </div>
          ...
          <template v-if="canManage">
            ...
            <div class="pa-6 bg-grey-lighten-4 rounded-lg border">
              ...
              <div class="d-flex flex-column flex-sm-row ga-3">
                ...
                <v-btn class="flex-grow-1 text-none font-weight-bold" color="error" data-testid="delete-member-btn"
      ...
      <v-col v-if="authStore.isManagerOrAdmin" cols="12" lg="5">
        <RecentTransactions :filter-params="{ initiator_user_id: memberId }"
          :title="$t('teamMembers.recentTransactions')" :view-all-query="{ initiator_user_id: String(memberId) }" />
      </v-col>
    </v-row>
  </template>
