...
<template>
    ...
    <template v-else>
        <v-row>
                    ...
                    <v-form @submit.prevent="submit">
                        <div class="d-flex flex-column ga-6">
                            <v-text-field v-model="form.name" autofocus color="primary" data-testid="wallet-name-input"
                                ...
                                :label="$t('wallets.walletName')" :placeholder="$t('wallets.walletNamePlaceholder')"
                            ...
                            <div v-if="wallet" class="pa-4 bg-grey-lighten-4 rounded-lg border">
                                ...
                                <div class="d-flex align-center ga-2">
                                    ...
                                    <v-btn color="grey-darken-1" density="comfortable" icon="mdi-content-copy"
                                        size="x-small" variant="text" @click="copyAddress" />
                                </div>
                            </div>

                            <v-select v-model="form.currency" color="primary" data-testid="wallet-currency-select"
                                density="comfortable" :error-messages="errors.currency" hide-details="auto"
                                :items="currencies" :label="$t('wallets.baseCurrency')" required variant="outlined" />

                            <div class="d-flex ga-4 mt-4">
                                <v-btn class="text-none font-weight-bold px-8" color="primary"
                                    data-testid="wallet-save-btn" height="48" :loading="processing" rounded="lg"
                                    type="submit">
                                    {{ $t('wallets.saveChanges') }}
                                </v-btn>
                                <v-btn class="text-none font-weight-bold px-8" color="grey-darken-1"
                                    :disabled="processing" height="48" rounded="lg" to="/wallets/" variant="outlined">
                                    {{ $t('common.cancel') }}
                                </v-btn>
                            </div>
                            ...
                            <div class="pa-6 bg-grey-lighten-4 rounded-lg border">
                                ...
                                <div class="d-flex flex-column flex-sm-row ga-3">
                                    ...
                                    <v-btn class="flex-grow-1 text-none" color="error"
                                        :disabled="!wallet?.can_delete || processing" :prepend-icon="Trash2"
                                        rounded="lg" variant="tonal" @click="handleDelete">
            ...
            <v-col v-if="authStore.isManagerOrAdmin" cols="12" lg="5" order="1" order-lg="2">
                <v-card v-if="wallet" border class="pa-4 mb-4" flat rounded="lg">
                    <v-row dense>
                        <v-col cols="6">
