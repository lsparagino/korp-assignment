...
<template>
  <PageHeader :title="$t('wallets.title')">
    <div class="d-flex ga-2 align-center">
      <v-btn v-if="authStore.isAdmin" class="text-none font-weight-bold" color="primary" data-testid="create-wallet-btn"
        prepend-icon="mdi-plus" rounded="lg" to="/wallets/create" variant="flat">
        {{ $t('wallets.createWallet') }}
      </v-btn>
    </div>
  </PageHeader>

  <DataTable :loading="processing" :meta="meta" :refreshing="refreshing" :title="$t('wallets.title')" @refresh="refresh"
    @update:page="handlePageChange" @update:per-page="handlePerPageChange">
    <template #columns>
    ...
    <template #body>
      <tr v-for="w in wallets" :key="w.id" class="cursor-pointer" :data-testid="`wallet-row-${w.id}`"
        ...
        <td class="font-weight-bold">
          ...
          <v-icon v-if="w.available_balance !== w.balance" class="ms-1" color="amber-darken-2" icon="mdi-alert"
            size="14" />
        </td>
        <td>
        ...
        <td>
          <v-chip class="font-weight-bold text-uppercase" :color="getStatusColors(w.status).bg" size="small"
            variant="flat">
