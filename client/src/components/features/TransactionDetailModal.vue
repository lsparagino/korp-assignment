...
<template>
  <v-dialog max-width="560" :model-value="modelValue" @after-leave="resetState"
    ...
    <v-card v-if="transaction" rounded="lg">
      ...
      <div class="sliding-container">
        <div class="sliding-track"
            ...
            <v-card-text class="pa-5">
              ...
              <div class="d-flex flex-column flex-sm-row align-center align-sm-center justify-space-between ga-2 mb-6">
                <div class="d-flex align-center ga-1">
                  <v-chip class="text-uppercase font-weight-bold"
                    :color="getTransactionTypeColors(getTransactionLabel()).text" size="small" variant="text">
                    {{ getTransactionLabel() }}
                  </v-chip>
                  <v-chip class="text-uppercase font-weight-bold"
                    :color="getTransactionStatusColors(transaction.status).text" size="small" variant="text">
              ...
              <v-list class="pa-0" lines="two">
                ...
                <v-list-item class="px-0">
                  ...
                  <div v-if="getFromWallet()?.address" class="text-caption text-grey-darken-2 font-weight-regular mt-1"
                    style="font-family: monospace">
                    {{ getFromWallet()?.address }}
                  </div>
                  <div v-else-if="transaction.external && transaction.external_address"
                    class="text-caption text-grey-darken-2 font-weight-regular mt-1" style="font-family: monospace">
                ...
                <v-list-item class="px-0">
                  ...
                  <div v-if="getToWallet()?.address" class="text-caption text-grey-darken-2 font-weight-regular mt-1"
                    style="font-family: monospace">
                    {{ getToWallet()?.address }}
                  </div>
                  <div v-else-if="transaction.external && transaction.external_address"
                ...
                <template v-if="transaction.initiator">
                  ...
                  <v-list-item class="px-0" data-testid="initiator-row">
                    ...
                    <template v-if="isManagerOrAdmin" #append>
                      <v-btn class="text-none text-caption" color="primary" data-testid="view-recent-btn" size="small"
                        variant="text" @click.stop="showRecentTransactions">
              ...
              <template v-if="canReview()">
                ...
                <v-alert v-if="error" class="mb-4" color="error" data-testid="review-error" density="compact"
                ...
                <template v-if="!rejectMode">
                  <div class="d-flex ga-3">
                    <v-btn class="text-none font-weight-bold flex-grow-1" color="success" data-testid="approve-btn"
                      :loading="submitting" prepend-icon="mdi-check-circle" rounded="lg" variant="flat"
                      @click="handleApprove">
                      {{ $t('transactions.approve') }}
                    </v-btn>
                    <v-btn class="text-none font-weight-bold flex-grow-1" color="error" data-testid="reject-btn"
                      :disabled="submitting" prepend-icon="mdi-close-circle" rounded="lg" variant="outlined"
                ...
                <template v-else>
                  <v-textarea v-model="rejectReason" auto-grow class="mb-3" color="error"
                    data-testid="reject-reason-input" :label="$t('transactions.rejectReasonLabel')"
                    :placeholder="$t('transactions.rejectReasonPlaceholder')" rows="3" variant="outlined" />
                  <div class="d-flex ga-3">
                    <v-btn class="text-none font-weight-bold flex-grow-1" color="error" data-testid="confirm-reject-btn"
                      :disabled="!rejectReason.trim()" :loading="submitting" prepend-icon="mdi-close-circle"
                      rounded="lg" variant="flat" @click="handleReject">
                      {{ $t('transactions.reject') }}
                    </v-btn>
                    <v-btn class="text-none font-weight-bold" color="grey-darken-1" data-testid="cancel-reject-btn"
                      :disabled="submitting" rounded="lg" variant="outlined" @click="rejectMode = false">
                      >
                      {{ $t('common.cancel') }}
                    </v-btn>
                  </div>
              ...
              <template v-if="canCancel()">
                ...
                <v-alert v-if="error && !canReview()" class="mb-4" color="error" data-testid="cancel-error"
                  density="compact" type="error" variant="tonal">
                  {{ error }}
                </v-alert>

                <v-btn block class="text-none font-weight-bold mt-3" color="grey-darken-1"
                  data-testid="cancel-transaction-btn" :loading="submitting" prepend-icon="mdi-cancel" rounded="lg"
                  variant="outlined" @click="openConfirmDialog({
          ...
          <div class="sliding-page">
            <v-card-text class="pa-5">
              <v-btn class="text-none mb-4" color="primary" data-testid="back-to-details-btn"
                prepend-icon="mdi-arrow-left" size="small" variant="text" @click="backToDetails">
              ...
              <TransactionTable compact is-admin :items="recentTransactions" :loading="loadingRecent"
      ...
      <ConfirmDialog v-model="confirmDialog.show" :message="confirmDialog.message"
        :processing="confirmDialog.processing" :title="confirmDialog.title" @confirm="executeConfirm" />
    </v-card>
  </v-dialog>
</template>
