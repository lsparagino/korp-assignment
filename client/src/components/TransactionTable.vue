...
<template>
  <v-card border flat :loading="loading" rounded="lg">
    <v-card-title class="pa-4 bg-grey-lighten-5 border-b">
      <span class="text-subtitle-1 font-weight-bold text-grey-darken-3">{{ title }}</span>
    </v-card-title>
      ...
      <v-table density="comfortable">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td class="text-grey-darken-2 text-caption text-no-wrap">{{ formatDate(item.created_at) }}</td>
            ...
            <td :class="[getAmountColor(item), 'font-weight-black']">
              {{ formatAmount(item) }}
            </td>
            <td>
